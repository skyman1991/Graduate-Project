...
uint8 Scan_Channel(uint8 startch,uint8 endch)
{
    ...
    for(i=startch;i<endch+1;i++)   //循环16次
    {
        ...
        if(timeout>0)
        {
            ...
            ScanChannel[i-1].cluster_id = data[2];
        }
        else
        {
...
void CreatJoinRequest()
{
    ...
    DataSendBuffer[2] = JoinRequestPacket.phy_address>>24;
    DataSendBuffer[3] = JoinRequestPacket.phy_address>>16;
    DataSendBuffer[4] = JoinRequestPacket.phy_address>>8;
    DataSendBuffer[5] = JoinRequestPacket.phy_address;
    DataSendBuffer[6] = JoinRequestPacket.des_cluster_id;
    DataSendBuffer[7] = JoinRequestPacket.des_cluster_innernum;
    DataSendBuffer[8] = 0;
    DataSendBuffer[9] = 0;
    DataSendBuffer[10] = 0;
...
void CreatJoinRequestACKOK()
{
    ...
    DataSendBuffer[2] = JoinRequestACKOKPacket.src_cluster_id;
    DataSendBuffer[3] = JoinRequestACKOKPacket.src_cluster_innernum;
    DataSendBuffer[4] = JoinRequestACKOKPacket.des_cluster_id;
    DataSendBuffer[5] = JoinRequestACKOKPacket.des_cluster_innernum;
    DataSendBuffer[6] = 0;
    DataSendBuffer[7] = 0;
    DataSendBuffer[8] = 0;
    DataSendBuffer[9] = 0;
...
void JoinRequestACKHandler()
{
    ...
    {
        EndPointDevice.cluster_id = DataRecvBuffer[8];
        EndPointDevice.cluster_innernum = DataRecvBuffer[9];
